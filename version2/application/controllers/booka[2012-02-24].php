  ...
  function gig_profile($id=0,$profile_id=0) {
   ...
   // $this->form_validation->set_rules('event_occ', 'Event Occurence', 'required|is_numeric');
    ...
    function paypal_process($gig_id,$profile_id) {    
            ...
            //seleting gig values            		
             $sql_gig = $sql = "select * from manage_gigs gig JOIN artist_gig_map artmap  ON artmap.gig_id = gig.gig_id JOIN artist art ON  artmap.artist_id = art.id where gig.gig_id  = '{$gig_id}' and art.id='{$profile_id}'";		                
                $r = $this->db->query($sql_gig);
                $data=$r->result_array();               
           ...
           //$site_fee =$site_fee+$amount;
            ...
            $bodyparams = array ("requestEnvelope.errorLanguage" => "en_US",
                        ...
                        "receiverList.receiver(1).email" => $data[0]['paypal_email'], //TODO
