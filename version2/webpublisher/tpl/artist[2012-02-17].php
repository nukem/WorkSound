                        ...
                        <form action=".?id=<?php echo $_REQUEST['id'] ?>" method="post" enctype="multipart/form-data" id="search" > 
                            <table width="95%" style="color:black; margin: 30px 30px 0px 30px">
                                ...
                                <tr id="">
									...
									<?php if($_SESSION['acx']==1) echo '<option value="test">Test</option>'; ?>
											<option value="new" <?php if('new'==$_REQUEST['join_status']) echo 'selected="selected"';  ?>>New</option>
											<option value="approved" <?php if('approved'==$_REQUEST['join_status']) echo 'selected="selected"';  ?>>Approved</option>
											<!--<option value="wait for 3 months" <?php if('wait for 3 months'==$_REQUEST['join_status']) echo 'selected="selected"';  ?>>Wait 3 Months</option>-->
											<option value="reject" <?php if('reject'==$_REQUEST['join_status']) echo 'selected="selected"';  ?>>Reject</option> </select>
								...
								?>
                                    <tr id="<?php echo $artist['id'];?>">
                                     <td width="18%"  style="padding: 0px"><a href=".?id=<?php echo $_REQUEST['id'] ?>&aid=<?= $artist['id'] ?>"><?= $artist['first_name'] . " " . $artist['last_name'].' ('.$audio.','.$video.')' ?></a></td>
										...
										<td  style="padding: 0px"><select name="status" id="join_status" onChange="javascript: 
                ...
                <div style="min-height:500px;">
                    <?  if (isset($_REQUEST['aid']) && $_REQUEST['aid'] != '') { ?>
						...
						<input type="hidden" id="parent_id" onchange="alert('passed');">
							...
							class="button" onclick="window.open('<?=$cfg['website_url'] ?>/user/directLogin/<?php echo $artist_data['user_id']; ?>?url=profile/view/<?php echo $profile_name_url; ?>','mywindowprofile','menubar=1,resizable=1,width=600,height=600,left=500,top=300,scrollbars=1');"/>
							
							
							<input type="button" name="editprofile" id="cal-btn" value="Edit Profile" 
							class="button" onclick="window.open('<?=$cfg['website_url'] ?>/user/directLogin/<?php 
							echo $artist_data['user_id']; ?>','mywindowedit','menubar=1,resizable=1,width=850,height=600,left=300,top=300,scrollbars=1');"/>
