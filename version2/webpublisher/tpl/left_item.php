<?php
  ...
  $state_sql = ($path[2][1] != 'Default') ? 'AND (state = "'.$path[2][1].'" OR state IS NULL)' : '';

  $promotions = dbq ("SELECT wp_structure.id, title, state
    FROM wp_structure, wp_promotion
    LEFT JOIN wp_states ON wp_promotion.state_id = wp_states.id
    WHERE wp_structure.id=link AND online = '1'
    $state_sql
    ORDER BY position");
?>
    ...
    <div id="right-col"> 
      ...
      <form action=".?id=<?= $id ?>" method="post" enctype="multipart/form-data" > 
		...
		<td>
		  ...
		  <input type="text" name="link_title" class="textfield width-100pct" value="<? if (isset ($_POST['link_title'])) echo htmlspecialchars ($_POST['link_title']); else echo htmlspecialchars ($record['link_title']); ?>" />
		</td>
		<td colspan="3">
		  <label>Link URL</label><br />
		  <input type="text" name="link_item" class="textfield width-100pct" value="<? if (isset ($_POST['link_item'])) echo htmlspecialchars ($_POST['link_item']); else echo htmlspecialchars ($record['link_item']); ?>" />
		</td>
	      </tr>
	      <tr style="display:none;">
		<td colspan="2" style="display:none;">
		  <label>Promotion</label><br /> 
		  <select id="promotion_id" name="promotion_id" class="textfield width-100pct">
		    <option value=""></option>
		    <?php if(!empty($promotions)): ?>
		      <?php foreach($promotions as $promotion): ?>
			<option value="<?php echo $promotion['id']; ?>" <?php if((isset($_POST['promotion_id']) && $_POST['promotion_id'] == $promotion['id']) || $record['promotion_id'] == $promotion['id']) echo 'selected="selected"'; ?>>
			  <?php echo $promotion['title'].' '; echo ($promotion['state'] == '') ? '(ALL)' : '('.$promotion['state'].')' ?>
			</option>
		      <?php endforeach; ?>
		    <?php endif; ?>
		  </select>
		...
		<td colspan="2" style="display:none;"></td>
		  ...
		  <textarea name="content" class="textfield height-200 width-100pct tinymce"><? if (isset ($_POST['title'])) echo htmlspecialchars ($_POST['content']); else echo htmlspecialchars (preg_replace('/src="/', 'src="../', $record['content'])); ?></textarea>
                </td>
		<td colspan="2"></td>
	      </tr>
	      <tr>
		<td colspan="2">
	      ...
	      <td colspan="2">
		<label>Upload Video Files</label><br />
