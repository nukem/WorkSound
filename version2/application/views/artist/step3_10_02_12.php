				...
				<div class="form_block">
					<table class="instrument_table" id="tbl_audio">
					  ...
					  <tr class="<?=($ix%2) ? 'tr_even' : ''?>" id="tr_<?=$ma->id?>">
					    ...
					    <td><a href="<?=$ma->url?>" class="<?php if (strpos($ma->url, 'soundbooka.com.au') !== false) echo 'lightbox_link ';?>link" rel="audio" target="_blank"><?=character_limiter($ma->url,40)?></a></td>
				...
				<div class="form_block">
					<table class="instrument_table"  id="tbl_video">
					  ...
					  <tr class="<?=($ix%2) ? 'tr_even' : ''?>" id="tr_<?=$mv->id?>">
					    ...
					    <td><a href="<?=$mv->url?>" class="<?php if (strpos($mv->url, 'soundbooka.com.au') !== false) echo 'lightbox_link ';?>link" rel="video" target="_blank"><?=character_limiter($mv->url,50)?></a></td>
	...
	$('.pic-remove').live('click',function() {
		...
		$.post("<?=base_url()?>ajax/removeProfilePic", {id: $(this).attr('rel')},
		   function(data) {
			 ...
			 //elm.closest('li').html('<img src="<?=base_url()?>images/default_person.gif" alt="" /> <span><a>Set as profile</a></span> <span><a>Remove</a></span>');
...
function ajaxFileUploadImg(elemID, parentID)
	...
	$(function(){
		$('#tbl_audio tbody').sortable({
			update: function(e, ui){
				var order = $(this).sortable('toArray').toString();
				$.post('<?=base_url()?>ajax/update_audio_position',{'order':order},function(data){
				});
			}
		}); 
	});
	...
	$(function(){
		$('#tbl_video tbody').sortable({
			update: function(e, ui){
				var order = $(this).sortable('toArray').toString();
				$.post('<?=base_url()?>ajax/update_video_position',{'order':order},function(data){
				});
			}
		}); 
	});
