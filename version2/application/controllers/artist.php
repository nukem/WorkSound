...
//
  ...
  function _sendEmail($id) {
	...
	$article = $this->mArticle->getEmailTemplate(194220);
		...
		$captcha=$this->random_text();
		$profile_name = $artist->profile_name;
		//$profile_link = 'Your <b>Username</b> is : '. $user->email .'<br/>';
		//$profile_link .='Your <b>Password</b> is: '. $user->password .'<br/>';
		//$profile_link = "<a href='" . base_url() . "artist/step1/{$id}'>Create my profile</a>";
		$profile_link = "Your <b>Username</b> is : ". $user->email ."<br/><br/>Your <b>Password</b> is : ". $user->password ."<br/><br/>To Login to your account click <a href='" . base_url() . "user/directLogin/{$id}?clr={$captcha}' style=\"font-weight:bold;text-decoration:underline;\">here</a>";
	
	$html_content = str_replace('%Profile name%',$profile_name,$article->content);
	$html = str_replace('%Create my profile%',$profile_link,$html_content);
	...
	send_email($user->email, $article->subject, $html);
    return;
  }
   ...
   private function _doUpload($id) {
	  ...
	  function settings($id){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			(
				'title' => 'Settings',
				'main_content' => 'artist/settings'
			);
			$data['form_errors'] = $this->form_validation->error_array();
			$sql = "select freq_of_email,booking_notify from artist where id = '{$id}' ";
			...
			$sql = sprintf
			(
				"UPDATE `artist` SET freq_of_email = '%s', booking_notify = '%s' WHERE	`id` = '%s'",
				$_POST['freq_of_email'],
				$_POST['booking_notify'],
				$id
			);
			$this->db->query($sql);
			redirect('/artist/settings/'.$id);
		}
	  }
	  ...
	  function manage_gig_list($id){
		...
		$data = array(
		  'title' => 'Manage Gigs',
		  'main_content' => 'artist/manage_gig'
		);
		$data['types'] = $this->mUtil->getProfileTypeList();
		$data['id']=$id;
		$retrieve=array('Confirm', 'Offer', 'Accepted', 'Rejected');
		$retrieve=implode("','",$retrieve);
		$r = $this->db->query("SELECT *,agm.id AS map_id FROM artist_gig_map agm, manage_gigs a ,artist ar
								WHERE  a.gig_id=agm.gig_id && agm.artist_id='{$id}' && ar.id='{$id}' && agm.offer_status IN ('{$retrieve}')");
    ...
    function notification($id){
		...
		$data = array(
			'title' => 'Notification',
			'main_content' => 'artist/notification'
		);
		//count for notification
        $sql_count = "select count(*) n from notifications noti LEFT JOIN manage_gigs gig ON noti.gig_id = gig.gig_id JOIN artist_gig_map agm ON agm.gig_id = gig.gig_id where noti.artist_id = '{$id}' and noti.is_view= '0'";
		...
		$sql = "select *,noti.status as notify_status,noti.created as notify_created from notifications noti JOIN manage_gigs gig ON noti.gig_id = gig.gig_id where noti.artist_id  = '{$id}' ";
		...
		artist art ON  noti.artist_id = art.id where noti.artist_id  = '{$id}' order by gig.gig_name";
    ...
    function view_notification($id) {
		redirect('/artist/notification/'.$this->session->userdata('artist_id'));
	}
