...
<?php class submit {
	  ...
	  $sql = "select artist.*, artist_type.type as artist_type_name,usr.email from artist 
            ...
            where usr.email LIKE '%" . $_POST['email'] . "%' AND (first_name LIKE  '%" . $_POST['name'] . "%' OR last_name LIKE '%" . $_POST['name'] . "%') AND usr.email LIKE '%" . $_POST['email'] . "%' AND phone_number LIKE '%" . $_POST['phone'] . "%' AND (artist.status LIKE '%" . $_POST['join_status']. "%') ";
if(isset($_POST['type']) && $_POST['type'] != ''){
    $sql .= " AND profile_type = '" . $_POST['type'] . "'";
}
                ...
                <div style="min-height:500px;">
                    <?  if (isset($_REQUEST['aid']) && $_REQUEST['aid'] != '') { ?>
						...
						<input type="hidden" id="parent_id" onchange="alert('passed');">
							...
							class="button" onclick="window.open('http://soundbooka.com/version1/artist/step1/<?php 
