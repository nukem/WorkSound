$(function(){
	var BASE_URL = 'http://soundbooka.com.au/version2/';
	...
	$('.history_view').click(function(){
		  ...
		  //$.post(baseurl + 'booka/notification/' + $id ,{},function(data){
		   $box.append(data);
		  });
	...
	$('.rate_view').click(function(){
	  ...
	  //$.post(base_url+'admin_jquery/view/'+$alt+'/'+$id,{},function(data){
	  $.post(baseurl + 'ajax/rate/' ,{'rel':$rel},function(data){
	   $box.append(data);
	  });
	...
	$('.calender_view').click(function(){
		...
		$fade=$('<div></div>').addClass('fade')
				.css({'opacity':'0.75', 'position':'fixed', 'z-index':9997, 'top':0, 'left':0, 'width':'100%', 'height':'100%', 'background-color':'#FFFFFF'});
		$close=$('<a></a>').addClass('close').html('<img width="25" src="'+ baseurl + 'images/window-close.png" />').attr('href','javascript:void(0);')
		.css({'font-size':'19px', 'position':'fixed', 'z-index':9998, 'top':0, 'right':0, 'margin':'53px 360px 0 0'});
		$box=$('<div></div>').addClass('box')
				.css({'position':'fixed', 'z-index':9999, 'top':'75px', 'left':'35%', 'width':'33%', 'height':'auto', 'background-color':'#FFFFFF', 'border': '1px solid #333','padding':'10px','overflow':'auto'});
