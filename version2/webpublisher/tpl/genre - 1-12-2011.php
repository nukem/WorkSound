...
class Artist { 
	function add_editGenre($runat,$genre_id=0,$genre='',$description='',$artist_type=''){
		ob_start();
		switch($runat){
		case 'local':
		...
		case 'server': 
					if($genre_id > 0)
						$sql="update genre set artist_type='$artist_type', genre='$genre', description='$description' where genre_id='$genre_id'";
					else 
	...
	function updateActive($genre_id,$value){
		ob_start();
		mysql_query("update genre set active='$value' where artist_id='$genre_id'");
		$html = ob_get_contents();
		ob_end_clean();
		return $html;
	}
 		 	 ...
 		 	 while($row=mysql_fetch_array($result)){?>
				...
				<td ><?php 
				 ...
				 while($row1=mysql_fetch_array($result1)){
							...
							onclick="artist.add_editGenre('local','<?php echo $row1['genre_id'];?>',{target:'frm'});"><?php echo $row1['genre']; ?></a>
