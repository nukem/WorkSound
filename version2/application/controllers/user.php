		...
		if ($a->num_rows()) {
			...
			//if ($artist->status == 'approved') {
				...
				//redirect('artist/step1/' . $artist->id); 
				     if(strstr($_SERVER['HTTP_REFERER'], 'profile')){				
	 ...
	 if ($q->num_rows()) {
	  ...
	  if ($a->num_rows()) {
			if($type==0){
				...
				$booka = $a->row();
				//print_r($booka);
				$this->session->set_userdata('artist_id',$booka->id); // found these lines way off track Max 7/3/2012
				$this->session->set_userdata('artists',$arr); // found these lines way off track Max 7/3/2012
