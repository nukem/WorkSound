...
class Ajax extends MY_Controller {
  ...
  public function getStateOptions($country_id,$state_id=0) {
    $states = $this->mUtil->getStateList($country_id);
	
	$state_value = $this->session->userdata('searching');
	
    $options = array('' => '<option value="">-- please select --</option>');
    foreach ($states as $k => $s) {
	if(@$state_value[0]['state'] == $k || $state_id == $k) $selected = 'selected=selected';
	else $selected = '';
      $options[] = "<option value='{$k}' {$selected} >{$s}</option>";
    }
    echo implode("\n", $options);
    die;
  }
  ...
  public function getInstrumentOptions($cat_id) {
	...
	function update_artist_gig_status(){
        ...
        //artist phone number        
		$artist_sql = "SELECT * FROM artist WHERE id = {$histroy_data['artist_id']}";
        $artist_ph = $this->db->query($artist_sql);
        $artistdetail = $artist_ph->result_array();       
        $artist_phone = $artistdetail[0]['phone_number'];
        //sending SMS to the artist      
         /* $html=$email->sms_content;        
            $si = new SmsInterface (false, false);
		...
		$sql_to = "select u.email from booka b JOIN user u ON b.user_id = u.id where b.id = ".$histroy_data['booka_id']." ";
