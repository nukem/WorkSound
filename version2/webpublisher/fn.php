...
function in_array_deep($needle, $haystack, $key = false) {  // thanks vandor at ahimsa dot hu (php.net) for the idea
...
function resize_img ($oldFile, $newFile, $newWidth, $newHeight, $method, $stretch, $jpgQuality, $fillR, $fillG, $fillB) {
 ...
 if ($method == 'shrink') {
  ...
  if (! $stretch && ($imageInfo[0] < $newWidth || $imageInfo[1] < $newHeight)) {
   ...
   if ($imageInfo[0] / $imageInfo[1] >= $newWidth / $newHeight)
     imagecopyresampled ($newImage, $oldImage, -((($imageInfo[0] * $newHeight) / $imageInfo[1]) - $newWidth)/2, 0, 0, 0, ($imageInfo[0] * $newHeight) / $imageInfo[1], $newHeight, $imageInfo[0], $imageInfo[1]);
   else
     imagecopyresampled ($newImage, $oldImage, 0, -((($newWidth * $imageInfo[1]) / $imageInfo[0]) - $newHeight)/2, 0, 0, $newWidth, ($newWidth * $imageInfo[1]) / $imageInfo[0], $imageInfo[0], $imageInfo[1]);
  }
  imagejpeg ($newImage, $newFile, $jpgQuality);
 } elseif ($method == 'fit') {
  $newImage = imagecreatetruecolor ($newWidth, $newHeight);
...
function strip_accents ($str) {
 $translation = array ('Ã�' => 'A', 'Ã„' => 'A', 'ÄŒ' => 'C', 'ÄŽ' => 'D', 'Ã‰' => 'E', 'Äš' => 'E', 'Ã‹' => 'E', 'Ã�' => 'I', 'Å‡' => 'N', 'Ã“' => 'O', 'Ã–' => 'O', 'Å˜' => 'R', 'Å ' => 'S', 'Å¤' => 'T', 'Ãš' => 'U', 'Å®' => 'U', 'Ãœ' => 'U', 'Ã�' => 'Y', 'Å½' => 'Z', 'Ã¡' => 'a', 'Ã¤' => 'a', 'Ä�' => 'c', 'Ä�' => 'd', 'Ã©' => 'e', 'Ä›' => 'e', 'Ã«' => 'e', 'Ã­' => 'i', 'Åˆ' => 'n', 'Ã³' => 'o', 'Ã¶' => 'o', 'Å™' => 'r', 'Å¡' => 's', 'Å¥' => 't', 'Ãº' => 'u', 'Å¯' => 'u', 'Ã¼' => 'u', 'Ã½' => 'y', 'Å¾' => 'z');
 return strtr ($str, $translation);
}

function test_field($value) {
	// test the value
