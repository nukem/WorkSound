...
<div id="page"> 
  ...
  <div id="content"> 
    ...
    <div id="right-col"> 
      <h2 class="bar green"><span>Home and Land Package</span></h2> 
      <form action=".?id=<?= $id ?>" method="post" enctype="multipart/form-data" > 
		...
		<td>
		  <label>Suburb &bull;</label><br />
		  <select id="suburb_id" name="suburb_id" class="textfield width-100pct">
		    <option value=""></option>
		  </select>
		</td>
		<td>
...
?>
		  ...
		  <tr>
			<td colspan="4">
				  <fieldset>
					  ...
					  <p>
						<a href="../infokit/property_brochure/<?php echo $id; ?>/vic/">
							Download Property Brochure (VIC Template)</a>
							|
						<a href="../infokit/property_brochure/<?php echo $id; ?>/nsw/">
							Download Property Brochure (QLD / NSW Template)</a> 
							|
						<a href="../infokit/property_brochure/<?php echo $id; ?>/two_page/">
					  ...
					  <table class="rec-table">
						  <tr>
							  <td>
								  <label>
									  <input type="checkbox" name="fixed_site_cost" value="1" <?php if(show_value('fixed_site_cost') == 1) { echo 'checked="checked"'; } ?> />
									  Fixed Site Cost
								  </label>
							  </td>
							  <td>
		  ...
		  <tr>
			  <td colspan="4">
				  <fieldset>
					  <legend>Information for Real Estate Portals</legend>
					  <table class="rec-table">
						  <tr>
							  <td colspan="4">
								  <label><input type="checkbox" name="portal" value="1"  <?php if(show_value('portal') == 1) { echo 'checked="checked"'; } ?> /> Send this property to Real Estate Portals</label>
							  </td>
						  </tr>
						  <tr>
							<td>
...
foreach($options as $k => $o) {
	if($_POST['category'] == $k || $record['category'] == $k) {
		$selected = ' selected="selected" ';
	} else {
		$selected = '';
	}
...
?>
						  ...
						  <tr>
							  <td colspan="4">
								  <label><input type="checkbox" name="agent" value="1" <?php if (show_value('agent') == 1) { echo 'checked="checked"'; } ?> />Sale Managed by External Agent</label>
							  </td>
						  </tr>
						  <tr>
							  <td>
								  <label>Name</label><br />
								  <input type="text" name="agent_name" class="textfield width-100pct" value="<?php echo show_value('agent_name'); ?>" />
							  </td>
							  <td>
								  <label>Phone</label><br />
								  <input type="text" name="agent_phone" class="textfield width-100pct" value="<?php echo show_value('agent_phone'); ?>" />
							  </td>
							  <td>
								  <label>Mobile</label><br />
								  <input type="text" name="agent_mobile" class="textfield width-100pct" value="<?php echo show_value('agent_mobile'); ?>" />
							  </td>
							  <td>
								  <label>Email</label><br />
								  <input type="text" name="agent_email" class="textfield width-100pct" value="<?php echo show_value('agent_email'); ?>" />
							  </td>
						  </tr>
...
<tr>
  ...
  <script type="text/javascript">
    ...
    function change_state(state_id, suburb_id, home_id) {
      $("#suburb_id").children().remove();
	...
	$.post(
	  ...
	  function(data){
	    ...
	    if(data.options != 'no_data') {
		  ...
		  /*
		  $('#package_bed').val(data.home_data.bedrooms);
		  $('#package_bath').val(data.home_data.bathrooms);
		  $('#package_garages').val(data.home_data.carspaces);
		   */
		  ...
		  if ($.trim($('#package_garages').val()) == '') $('#package_garages').val(data.home_data.carspaces);
    ...
    $(function () {
      ...
      var load_suburb_id = '<?php if(isset($_POST['suburb_id'])) echo $_POST['suburb_id']; else echo $record['suburb_id']; ?>';
      ...
      change_state(load_state_id, load_suburb_id, load_home_id);
	...
	change_state($(this).val(), '', '');
	  ...
	  $('#suburb_id').change( function () {
		  $('#suburb').val($('#suburb_id :selected').text());
		  $('#postcode').val($('#suburb_id :selected').attr('rel'));
	  })
	...
	$('#facade_id').change(
		function() {
			$('.preview input').click();
			$.post (
				"copy_image_gallery_item.php",
				{'id': <?php echo $id; ?>, 'img_id': $(this).val()},
				function(data) {
					var liHTML = '<li class="sort-li" id="' + data.insert_id + '"><img src="js/handle.gif" alt="move" class="move" /><img src="js/edit.gif" alt="edit" class="edit" onclick="$(this).siblings(\'.editor\').css(\'display\', \'inline\'); $(this).siblings(\'.preview\').css(\'display\', \'none\'); trapEnter(\'#edit-' + data.insert_id + '\', ' + data.insert_id + ', \'image\');" /><span class="editor"><input type="text" id="edit-' + data.insert_id + '" value="' + data.image_title + '" /><input type="button" value="save" onclick="saveTitle(\'#edit-' + data.insert_id + '\', ' + data.insert_id + ', \'image\');" /><img src="js/loading.gif" alt="loading" class="edit-no-show" /><input type="button" id="edit-cancel-' + data.insert_id + '" value="cancel" onclick="$(this).parent().siblings(\'.preview\').css(\'display\', \'inline\'); $(this).parent().css(\'display\', \'none\');" /> or remove this image <img src="js/unlink.gif" alt="un-link" class="unlink" onclick="if (window.confirm (\'Are you sure?\')) { deleteFile(\'image\', ' + data.insert_id + '); $(this).parent().parent().hide(\'fast\'); }" /></span><a href="#" title="preview" class="preview" onclick="$(\'img.thumb:visible\').slideUp(200); $(this).siblings(\'img.thumb:not(:visible)\').slideDown(200); return false;"><span class="pic-title">' + data.image_title + '</span>&nbsp;<img src="js/preview.gif" alt="preview" /></a><span class="preview"><input type="checkbox" checked="checked" id="online-image-' + data.insert_id + '" onchange="updateOnline(\'image\', ' + data.insert_id + ');" /><img src="js/web_offline.gif" class="onoff" alt="online/offline" /></span><img src="../wpdata/images/' + data.insert_id + '-s.jpg" class="thumb" onclick="$(this).slideUp(100); return false;" /></li>';
					$('#image-sort')
						.append(liHTML)
						.SortableAddItem(document.getElementById(data.insert_id));
				},
				'json'
			)
		}
