 ...
 * Soundbooka
 * 
 * @author     Arunkumar Sivasubramanian
 */
class mBooka extends CI_Model {
  	...
  	$id = $form_data['id'];
   	if($id > 0){
   		$this->db->where('id', $id);
			...
			$this->db->insert('user_event', $form_data);
			$ret['IsSuccess'] = true;
  ...
  function addDetailedCalendar($st, $et, $sub, $re, $dscr, $loc, $radio_daily,$daily,$select_weekly,$weekly,$select_monthly,$monthly,$select_yearly,$selectmonth,$occur,$starttime,$endtime,$flag,$repeat_start_date){
	 if($flag=="0")
	 {
	  	$ret = array();
			...
			if(isset($form_data['repeater'])&& trim($form_data['repeater'])=="1")
			{
				$arr=explode("/",$form_data["start_time"]);
				$tempdt2=$arr[2]."-".$arr[1]."-".$arr[0];		
				
				$arr=explode("/",$form_data["event_to_date"]);
...
function other_profile($email){
	...
	$res=$this->db->query("SELECT a.id as id, u.email,a.first_name,a.last_name,a.booka_type from booka a, user u WHERE u.email='{$email}' AND a.user_id=u.id");
	$profiles=$res->result_array();
	return $profiles;
  }
}
...
function insertdetailevent($strFieldsName,$strInsertValue)
{
   ...
   function save($form_data,$data=array()) {
    $newId = 0;
	  ...
	  if($form_data['id'] != 0)
	  {
        ...
        $this->db->update('booka', $form_data);
		$newId = $form_data['id'];
	  }else{
	  
	  $form_data['created'] = date('Y-m-d H:i:s');
	  $this->db->insert('booka', $form_data);
 ...
 function save_manage_gigs($form_data,$data=array()) {
    $newId = 0;
   
	  if(!empty($form_data['gig_id']) && $form_data['gig_id'] != 0)
	  {
		$this->db->where('gig_id', $form_data['gig_id']);
		$form_data['modified'] = date('Y-m-d H:i:s');
        $this->db->update('manage_gigs', $form_data);
		$newId = $form_data['gig_id'];
	  }else{
	  
	  $form_data['created'] = date('Y-m-d H:i:s');
	  $this->db->insert('manage_gigs', $form_data);
	  $newId = $this->db->insert_id();
	  }
	    return $newId;
  }
  function get_gig_profile($id=0){
	if($id==0) return;
	$q=$this->db->get_where('manage_gigs',array('gig_id'=>$id));
	if ($q->num_rows() > 0) {
		$res=$q->result_array();
		$gig=array();
		if(count($res)>0)	{
			$gig= $res[0];
			$gig['start_date']=date('d/m/Y',strtotime($gig['start_date']));
			$gig['end_date']=date('d/m/Y',strtotime($gig['end_date']));
		}
		return $gig;
	}
  }
 ...
 function other_profile($email){
	$res=$this->db->query("SELECT a.id as id, u.email,a.firstname,a.lastname,a.booka_type from booka a, user u WHERE u.email='{$email}' AND a.user_id=u.id");
	$profiles=$res->result_array();
	#echo '<pre style="display:none">';print_r($profiles);echo '</pre>';
	return $profiles;
  }
}
