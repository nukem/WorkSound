   ...
   private function _doUpload($id) {
	  ...
	  function manage_gig_list($id){
		...
		*/
		...
		$sql = "SELECT *,agm.id AS map_id FROM artist_gig_map agm, manage_gigs a ,artist ar WHERE a.gig_id=agm.gig_id && agm.artist_id='{$id}' && ar.id='{$id}' && agm.offer_status IN ('{$retrieve}') ";
		...
		$sql .= "and   start_date >= '{$sdate}'  and end_date <='{$edate}'";
		
		if(@$_POST['status']=='Deposit Paid') $sql .= " and agm.respond_status like  'Deposit Paid' ";
		else if(@$_POST['status']=='Accept') $sql .= " and agm.respond_status like  'Accepted' ";
		else if(@$_POST['status']=='Reject') $sql .= " and agm.respond_status like  'Rejected' ";
		else if(@$_POST['status']=='Offer') $sql .= " and agm.offer_status =  'Offer' ";
		else if(@$_POST['status']=='Confirm') $sql .= "and agm.offer_status like  'Confirm' ";
		
		}
		else{				
			$sql = "SELECT *,agm.id AS map_id FROM artist_gig_map agm, manage_gigs a ,artist ar
									WHERE  a.gig_id=agm.gig_id && agm.artist_id='{$id}' && ar.id='{$id}' && agm.offer_status IN ('{$retrieve}')";
		}
