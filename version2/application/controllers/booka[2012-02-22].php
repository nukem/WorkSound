...
class Booka extends MY_Controller {
  ...
  function Booka() {
    ...
    //paypal adaptive
    $this->load->model('adaptive_payments');
  ...
  function gig_profile($id=0,$profile_id=0) {
	...
	// $data['allgenres']=$this->mArtist->allgenres();
      ...
      if ($newID = $this->mBooka->save_manage_gigs($form_data)) {
        ...
        if($this->input->post('payment_method')==1){
            if($this->adaptive_payments->Pay()){
                   $this->session->set_flashdata('message', 'You have successfully done paypal payment process!.');
            }else{
                  $this->session->set_flashdata('error', 'There is some problem with the payment Process.Please try again!');
            } 
          }
