...
//die('OK');  
  ...
  function personal_info($id){
		...
		$_POST['secret_question'] = addslashes($_POST['secret_question']);
  ...
  function gig_profile($id=0,$profile_id=0,$search=0) {
	...
	// $data['allgenres']=$this->mArtist->allgenres();
		if(!empty($_POST)){
			...
			$this->form_validation->set_rules('start_min', 'Start Min', '');
			$this->form_validation->set_rules('end_date', 'End Date', 'required|trim');
			$this->form_validation->set_rules('end_time', 'End Time', '');
			$this->form_validation->set_rules('end_time', 'End Min', '');
...
//			$this->form_validation->set_rules('genre', 'Genre', 'required');
		...
		if($profile_id>0) { 
			...
			$st_time = explode(':',$data['start_time']);
			$data['start_time'] = $st_time[0];
			$data['start_min'] = $st_time[1];
			$end_time = explode(':',$data['end_time']);
			$data['end_time'] = $end_time[0];
			$data['end_min'] = $end_time[1];
		}
		
		$e1 = $this->db->query("SELECT DISTINCT(gig_name) FROM manage_gigs where booka_id ='{$id}'");
      ...
      $this->load->view('template', $data);
		...
		$smin = $_POST['start_min'] * 5;
		$emin = $_POST['end_min'] * 5;
		$start_time=set_value('start_time').':'.$smin.':00';
		$end_time=set_value('end_time').':'.$emin.':00';
      $form_data = array(
      ...
      if ($newID = $this->mBooka->save_manage_gigs($form_data)) {
       ...
       $this->session->set_flashdata('message', 'You have successfully created a gig on Soundbooka. Please click â€œOfferâ€� to send this gig to the artist you have requested');
	...
	function manage_favourite($id=0){
		...
		// This structure must take from either artist or booka for both modes Max - before was showing all 07/03/2011
		if($this->session->userdata('profile_type') != 'booka'):
			$glob_artist_id = $this->session->userdata('artist_id');
			$q=$this->db->query("SELECT *,mf.status AS fstatus FROM manage_favourite mf, artist a WHERE a.id=mf.artist_id and mf.artist_id = $glob_artist_id");
		else:
			$glob_booka_id = $this->session->userdata('booka_id');
			$glob_artist_id = $this->session->userdata('artist_id');
			$glob_booka_id = $glob_artist_id;
			$q=$this->db->query("SELECT *,mf.status AS fstatus FROM manage_favourite mf, artist a WHERE a.id=mf.artist_id and mf.booka_id = $glob_booka_id");
		endif;
		$data['favourites']=$q->result_array();
		$this->load->view('template', $data);
		// $varerror = mysql_error(); $rows=mysql_num_rows($favourites);
	}
