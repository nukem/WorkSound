...
class GCalendar {
	...
	function add_event($runat)
	{
		...
		?>
				...
				<tr>
					<td> Event title:<br><input type="text" name="event_title" id="event_title" value="<?php echo $this->event_title;?>" /></td>
				</tr>
				<tr>
					<td>Comments:<br><textarea name="content" id="content"><?php echo $this->content;?></textarea></td>
				</tr>
				<tr>
					<td>Start (YYYY-MM-DD):<br><input name="start_date_display" type="text"  id="start_date_display" value=""/>
				...
				<tr>
				<td>End (YYYY-MM-DD)<br><input type="text" name="end_date_display" id="end_date_display" value="" />
					<script type="text/javascript">
				...
				<tr>
				<td align="right"><input name="submit" style="width:auto;" type="submit" value="add event" onClick="return <?php echo $ValidationFunctionName ?>();"/></td>
				</tr>
				</table>
				</form>
		<?
		...
		case 'server' :
					...
					$sdate=explode('-',$start_date_display);
					$this->start_date=$sdate[2].'-'.$sdate[1].'-'.$sdate[0];
					$edate=explode('-',$end_date_display);
					$this->end_date=$edate[2].'-'.$edate[1].'-'.$edate[0];
					 $sdate=explode('-',$this->start_date);	
						  $edate=explode('-',$this->end_date);	
						  $start = date(DATE_ATOM, mktime(date("H"), date("i"), 0, $sdate[1], $sdate[0], $sdate[2]));
						  $end = date(DATE_ATOM, mktime(date("H"), date("i"), 0, $edate[1], $edate[0], $edate[2]));
							
						  //$start = date(DATE_ATOM, strtotime($_POST['start_date_display']));
						  //$end = date(DATE_ATOM, strtotime($_POST['end_date_display']));
