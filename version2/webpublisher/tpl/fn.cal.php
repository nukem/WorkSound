...
<script type="text/javascript" language="javascript">
	// Date: 06 Dec'11 EM Code starts here
		...
		{
			window.open('tpl/edit_calendar.php?aid=','mywindow','menubar=1,resizable=1,width=500,height=450,left=500,top=300');
			return false;
		}
		
		function openwindow(e, eventid)
		{
			window.open('tpl/edit_calendar.php?aid=&eventid='+eventid,'mywindow','menubar=1,resizable=1,width=500,height=450,left=500,top=300');
...
function draw_calendar($month,$year,$events = array()){
  ...
  // Date: 30 Nov 2011 EM Code starts here
    //$calendar.= '<td class="calendar-day" id="calendarclick" onClick="window.open(\'tpl/edit_calendar.php?aid='.$aid.'\',\'mywindow\',\'menubar=1,resizable=1,width=450,height=450,left=500,top=300\');"><div style="position:relative;height:100px;">';
    $calendar.= '<td class="calendar-day" id="calendarclick" onClick="return openwindow1(event);"><div style="position:relative;height:100px;">';
    // EM Code ends here
     ...
     // $event_day = $list_day.'/'.$month.'/'.$year;
	       $qry = mysql_query("select u.id,u.event_title from user_event as u
							  left join user_event_detail as u_d
							  on u.id = u_d.parenteventid 
							  where u_d.startDate <= '".$event_day."' and u_d.endDate >= '".$event_day."'");
      ...
      <a href="#" onclick="return openwindow(event, \''.$eventid.'\');">'.$row_qry['event_title'].'</a>
      </div>';
      }
      ...
      $i++;
    	...
    	if($i > 4):
    	$calendar.= '<div class="event">
      <a href="#" onclick="return blockwindow(event);">More...</a>
      </div>';	
      endif;
