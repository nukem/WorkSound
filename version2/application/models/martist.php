...
function insertdetailevent($strFieldsName,$strInsertValue)
{
  ...
  function saveInstrument($data) {
        ...
        //getting count of intrument
        $countsql = "SELECT count(*) AS cntinstrument FROM artist_Instruments where `artist_id`='".$data['artist_id']."'";
        $resultcount = $this->db->query($countsql);
        $count=  $resultcount->result_array();
        //updating artist table
        $update_artist = "update artist set instrument_number ='".$count[0]['cntinstrument']."' where `id`='".$data['artist_id']."'";
        $this->db->query($update_artist);
		return $newId;
  }
  ...
  function searchArtist($post) {
	...
	if(!empty($post['instrument1']))  $sql.=" LEFT JOIN artist_Instruments ai ON a.id=ai.artist_id ";
	if(!empty($post['instrument2']))  $sql.=" LEFT JOIN artist_Instruments ai ON a.id=ai.artist_id ";
	if(!empty($post['pref_gigs'])) $sql.=" LEFT JOIN artist_gig ag ON a.id=ag.artist_id ";
	if(!empty($post['start_date']) && !empty($post['end_date'])) { 
        $sql.=" LEFT JOIN artist_gig_map amag ON amag.artist_id =a.id "; 
        $sql.=" LEFT JOIN manage_gigs mgig ON mgig.gig_id =amag.gig_id "; 
        $sql.=" LEFT JOIN artist_availability avail ON a.id =avail.artist_id"; 
        }
	$sql.=" WHERE 1=1 ";

	if(!empty($post['start_time'])) {
	if(!empty($post['start_hour']) && $post['start_hour'] == '1')
	$sttime = $post['start_time'] + 12;
	else 
        $sttime = $post['start_time'];
        $stt = $sttime.':00:00';
	}
	else $stt='00:00:00';
	if(!empty($post['end_time'])){
	if(!empty($post['end_hour']) && $post['end_hour'] == '1')
	$edtime = $post['end_time'] + 12;
	else 
	$edtime = $post['end_time'];
	$ent = $edtime.':00:00';
	}
	else $ent='00:00:00';
	 if(!empty($post['start_date']) && !empty($post['end_date'])) {
	...
	if($post['profile_type'] == 11) $sql.=" && a.profile_type in ('1','3','7','9',10)";
	else $sql.=" && a.profile_type='{$post['profile_type']}'";
	}
	...
	if(!empty($post['pref_gigs'])) $sql.=" && ( ag.gig_id != 0 ";
	if(!empty($post['pref_gigs'])){
        $i=0;
		foreach($post['pref_gigs'] as $val){        
        if($i==0){
        	$sql.=" && ag.gig_id = '{$val}'";	 
        }else{
        	$sql.=" || ag.gig_id = '{$val}'";	 
	...
	if(!empty($post['pref_gigs']))  $sql.=")";
	
	if(!empty($post['preferred_medium_arr'])) $sql.=" && ( a.preferred_medium != 'NULL'";
	if(!empty($post['preferred_medium_arr'])){
		foreach($post['preferred_medium_arr'] as $val){
			$sql.=" || a.preferred_medium LIKE '%{$val}%'";	 
		}
	}
	if(!empty($post['preferred_medium_arr']))  $sql.=")";
	
	if(!empty($post['specialization_arr'])) $sql.=" && ( a.specialization != ''";
	if(!empty($post['specialization_arr'])){
		foreach($post['specialization_arr'] as $val){
			$sql.=" || a.specialization LIKE '%{$val}%'";	 
		}
	}
	if(!empty($post['specialization_arr']))  $sql.=")";
	
	 if(!empty($post['instrument1'])) $sql.=" && ai.instrument_name LIKE '%{$post['instrument1']}%'";
	 if(!empty($post['instrument2'])) $sql.=" && ai.instrument_name LIKE '%{$post['instrument2']}%'";
	 if(!empty($post['artist_name_a'])) $sql.=" && a.profile_name LIKE '%{$post['artist_name_a']}%'";
	 if(!empty($post['artist_name_b'])) $sql.=" && a.profile_name LIKE '%{$post['artist_name_b']}%'";
	 if(!empty($post['artist_name_c'])) $sql.=" && a.profile_name LIKE '%{$post['artist_name_c']}%'";
	 if(!empty($post['artist_name_d'])) $sql.=" && a.profile_name LIKE '%{$post['artist_name_d']}%'";	 
	 if(!empty($post['suburb'])) $sql.=" && a.suburb LIKE '%{$post['suburb']}%'";
	 if(!empty($post['country'])) $sql.=" && a.country = '{$post['country']}'";
	 if(!empty($post['payment_method'])) $sql.=" && a.payment_method = '{$post['payment_method']}'";
	 if(!empty($post['state'])) $sql.=" && a.state = '{$post['state']}'";
	 if(!empty($post['postcode'])) $sql.=" && a.postcode LIKE '%{$post['postcode']}%'";
	
	 if(!empty($post['travel_city']))  $sql.=" && a.travel_city = '{$post['travel_city']}'";
	 if(!empty($post['travel_state']))  $sql.=" && a.travel_city = '{$post['travel_state']}'";
	 if(!empty($post['travel_interstate']))  $sql.=" && a.travel_city = '{$post['travel_interstate']}'";
	 if(!empty($post['travel_international']))  $sql.=" && a.travel_city = '{$post['travel_international']}'";
	 if(!empty($post['band_lineup'])){
     if($post['band_lineup'] < '6') { 
       $sql.=" && a.instrument_number  = '{$post['band_lineup']}' ";
     }else if($post['band_lineup'] >= '6'){ 
        $sql.=" && a.instrument_number >= '{$post['band_lineup']}' ";
     }
     }
	 //else{ $sql.=" && a.needed_equipment != 'NULL'"; } 
     ...
     $st=explode('/',$post['start_date']); $en=explode('/',$post['end_date']);
		...
		$en=$en[2].'-'.$en[1].'-'.$en[0].' '.$ent;
            if(!empty($post['start_date']) && !empty($post['end_date'])) {
                if(!empty($post['start_date']) && !empty($post['end_date'])) {$sql.=" && ( mgig.start_date >='{$st}' && mgig.end_date <= '{$en}' )"; }           
            }elseif(!empty($post['start_date']) && empty($post['end_date'])){
                $sql.=" && (mgig.start_date >= '{$st}' )";
            }elseif(empty($post['start_date']) && !empty($post['end_date'])) {
                $sql.=" && (mgig.end_date <= '{$st}')";
            }         
        }
     if(!empty($post['start_date']) && !empty($post['end_date'])) { 
                $start_str=explode('/',$post['start_date']);
                $startday = mktime(0, 0, 0, $start_str[1], $start_str[0], $start_str[2]);
                $startday_name = date("D", $startday) ;
            
             
                $endstr=explode('/',$post['end_date']);
                $endday = mktime(0, 0, 0, $endstr[1], $endstr[0], $endstr[2]);
                $endday_name = date("D", $endday) ;
            
         
             $sql.=" && ((avail.day LIKE '%$startday_name%') || (avail.day LIKE '%$endday_name%')) ";
           $edtime = $post['end_time'];
           $sttime = $post['start_time'];     
           if(!empty($sttime) && !empty($edtime)){
           if($sttime<='06'  && $edtime>='18'){
                $available_time ='day';
           }
           if($sttime<='18' && $edtime>='06'){
                $available_time ='night';
           }
            $sql.=" && ((avail.time LIKE '%{$available_time}%')) ";
            }
	 ...
	 if(!empty($post['fee_gig'])){
        $sql.="  && ( ( (a.fee_hour * a.gig_hours ) <= '{$post['fee_gig']}' && a.fee_gig = '0' ) || ( a.fee_gig <='{$post['fee_gig']}' && a.gig_hours='0' && a.fee_hour='0') ) ";
	 }
	 $sql.=" GROUP BY a.id";
