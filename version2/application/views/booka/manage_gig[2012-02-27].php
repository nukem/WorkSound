		...
		if(count($map_artists[$val['gig_id']]) > 0){ 
			...
			if(!empty($map_artists[$val['gig_id']]) && count($map_artists[$val['gig_id']])>0){
				...
				//else if($v['status']=='Fully Paid') {  $activate=1;}
					...
					//echo $status."&&&&&&&&&&&&";
					if($status=='Accepted' && $val['payment_method']==1) {
					...
					}else if($status=='Accepted' && $val['payment_method']==2) {                   ?>
					...
					if($activate==0  && $status!='Accepted' && ($status !='Deposit Paid' || $v['status'] !='Fully Paid')) { 
						echo '<td class="status" style="border-right:none;">';
						if(empty($gig_test[$val['gig_id']]) && $v['status'] !='Fully Paid') 
						...
						if($status=='Offer' ||  $v['status'] =='Fully Paid') 
