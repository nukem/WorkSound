  ...
  function gig_profile($id=0,$profile_id=0) {
	...
	// $data['allgenres']=$this->mArtist->allgenres();
      ...
      if ($newID = $this->mBooka->save_manage_gigs($form_data)) {
        ...
        //calling paypal adaptive here
        if($this->input->post('payment_method')==1){
        
            $this->paypal_process($gig_id,$profile_id);                 
            $this->session->set_flashdata('message', 'You have successfully done paypal payment process!.');                 
          }  
   ...
   // $this->form_validation->set_rules('event_occ', 'Event Occurence', 'required|is_numeric');
    ...
    function paypal_process($gig_id,$profile_id) {    
            ...
            $bodyparams = array ("requestEnvelope.errorLanguage" => "en_US",
                        "actionType" => "PAY",
                        "currencyCode" => "USD",
                        "cancelUrl" => $this->config->item('base_url')."booka/gig_profile/".$gig_id."/".$profile_id,
                        "returnUrl" => $this->config->item('base_url')."booka/gig_profile/".$gig_id."/".$profile_id,
            ...
            try
            {
                ...
                $sql ="UPDATE `manage_gigs` SET status='approved', paykey ='{$kArray["payKey"]}' WHERE gig_id ='{$gig_id}'";
                $this->db->query($sql);                                             
