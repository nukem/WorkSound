...
//print_r($_REQUEST);
	$query="SELECT * FROM booka b, user u where b.user_id=u.id && b.booka_type!=''";	
		...
		if($_REQUEST['type']!='') $query.=" && b.booka_type = '{$_REQUEST['type']}'";			
        if(isset($_REQUEST['join_status'])){  $query.=" && b.status = '{$_REQUEST['join_status']}'"; }
           // echo $query;
            $bookas = dbq ($query);	
                ...
                <div style="min-height:500px;">
                    <?php $business_name_url = str_replace(' ','_',$booka_data['business_name']); ?>
						...
						<input type="hidden" id="parent_id" onchange="alert('passed');">
							...
							class="button" onclick="window.open('<?=$cfg['website_url'] ?>/user/directLogin/<?php echo $bid; ?>/1?url=booka/manage_gig/<?php echo $booka_data['id']; ?>','mywindow','menubar=1,resizable=1,width=600,height=600,left=500,top=300,scrollbars=1');"/> 
							...
							echo $bid; ?>/1','mywindowedit','menubar=1,resizable=1,width=850,height=600,left=300,top=300,scrollbars=1');"/>
									...
									?>
										<tr>
											<td style="padding: 4px; margin: 2;" width="30%">
												<a href="?id=<?php echo($_REQUEST["id"]) ?>&type=<?php echo($row['id']) ?>" style="font-size: 14px;"><?php echo(ucwords($row['booka_type'])) ?></a>
											</td>
											<td style="padding: 4px; margin: 2;" width="50px">
												<a href="?id=<?php echo($_REQUEST["id"]) ?>&type=<?php echo($row['id']) ?>" style="font-size: 14px;"><?php echo($row['n']) ?></a>
											</td>
											<td>
